...
class Festivos extends Component {
  ...
  loadYears = () => {
    axios
      .get(PATH_FESTIVOS_SERVICE + "years")
      .then((result) => {
  ...
  loadFestivos = (ano) => {    
    if(ano == 'select'){
      this.setState({festivos: []});
    } else {
      axios
      .get(PATH_FESTIVOS_SERVICE + ano)
      .then((result) => {
  ...
  remove = async (id) => {
    await axios({
      ...
      .then(() => {
        let updatedFestivos = [...this.state.festivos].filter(
          (i) => i.id != id
        );
  ...
  save = async () => {
    ...
    await axios({
      ...
      .then((result) => {         
        fields.ano = Moment(fields.festivo).format('YYYY');
        this.loadFestivos(fields.ano)
        fields.festivo = "";
  ...
  render() {
    ...
    if (formState == "error") {
      ...
      messageLabel = (
        <Alert variant="success">El festivo se guardo satisfactoriamente</Alert>
      );
    } else if (formState == "deleted") {
      messageLabel = (
        <Alert variant="success">
          El festivo se elimino satisfactoriamente
        </Alert>
      );
    }
    ...
    if (errors.festivo) {
      messageFestivo = (
        <Alert variant="danger">{this.state.errors.festivo}</Alert>
      );
    }
    ...
    let optionAnos = anos.map((ano) => (
      <option defaultValue={ano} key={ano} value={ano}>
        {ano}
      </option>
    ));
    ...
    const modal = (
      <Modal
        ...
        <Modal.Body>Esta seguro de eliminar el festivo</Modal.Body>
        <Modal.Footer>
          <Button
          ...
          <Form className="form">
            <Col>
              <Container className="App">                
                <Row>
                  <Col sm="2">
                    <Form.Label>Fecha Festivo</Form.Label>
                  </Col>
                  <Col sm='3'>
                    <Datetime
                      ...
                      value={this.state.fields.festivo}
                    />
                    {messageFestivo}
                  </Col>
                </Row>
